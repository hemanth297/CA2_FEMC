function [ K , f ]  =  sub_assembly_SRI ( Model , Mesh , BC )
...
% D: 4-by-4
D  = sub_elasticity_matrix_mod ( Model.material );

%% Gauss point
int_pts = 2; %2 for full integration, 1 for reduced integration
GI = sub_Gauss_point_local_mod(int_pts);
K  =  zeros ( 2*num_node , 2*num_node );
f  =  zeros ( 2*num_node , 1 );

%% Domain integration - full Integration part
...
for idx_elem  =  1 : num_element
    ...
    % loop over Gauss point % full integration for Kd and F
    for idx_Gauss  =  1 : 4
        % shape functions
        ...
        % compute N and Bd matrices
        [ N , Bd ]  =  sub_get_N_and_Bd ( N0 , dN_dxi , x_node_element );
        ...
        K ( index_K , index_K )  =  K ( index_K , index_K )  +  ...
                                    Bd' * D * Bd * J * GI.weight_2d(idx_Gauss);
        f ( index_K )            =  f ( index_K )  +  ...
...
%% reduced integration for volumetric part
...
for idx_elem  =  1 : num_element
    ...
    for idx_Gauss  =  1 : 4
        ...
        % compute Bv matrix
        [ Bv ]  =  sub_get_Bv ( N0 , dN_dxi , x_node_element );
        % get Jacobian
        F = dN_dxi * x_node_element ; % based on iso-parametric mapping
        J = det(F);
        ...
        % Assemble
        K ( index_K , index_K )  =  K ( index_K , index_K )  +  ...
                                    Bv' * D * Bv * J * GI.weight_2d(idx_Gauss);
